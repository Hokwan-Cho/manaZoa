function Multiselect(item, opts) {
	...
	if ((typeof($jquery) != 'undefined' && !($jquery(item).is('select'))) ||
		(typeof($jquery) == 'undefined' && item.tagName != 'SELECT' != 'SELECT')) {
		throw "Multiselect: passed object must be a select";
	}
	
	if ((typeof($jquery) != 'undefined' && !$jquery(item).attr('multiple')) ||
		(typeof($jquery) == 'undefined' && !item.hasAttribute('multiple'))) {
...
Multiselect.prototype = {
	...
	_createItemList: function () {
		...
		var selectAll = this._createItem('span', -1, '전체');
